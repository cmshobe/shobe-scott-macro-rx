...
@nb.jit(nb.types.Tuple((nb.float64, nb.float64, nb.float64, nb.float64, nb.int8))(nb.float64, nb.float64,
                                                 nb.float64, nb.float64,
                                                 nb.float64, nb.float64,
                                                 nb.float64, nb.float64,
                                                 nb.float64, nb.float64,
...
#    tolerance_over_timestep = 2e-8
    ...
    which_regime_arr = cp.deepcopy(save_widths)
    ...
    memory_time = 10000 #seconds
#    prior_w = np.zeros(int(memory_time / timestep))
#    prior_S = np.zeros(int(memory_time / timestep))
    ...
    while kill_flag == 0 and time < time_to_run:
        ...
        R_r, S_r, d_r, f_r_over_f, which_regime = total_flow_resistance(Q, d_r, 
                                                                      wb, theta, 
                                                          a1, a2, sigma_z, 
                                                          g, S, chan_depth, 
        ...
        if fc_bank == 1:
            ...
            #D_bed = ((Qs_in) / reach_length) * (1 / (((1 / (k_dep)) * (np.power(tau_bed, 3/2) / np.power(tau_bank, 3/2))) * l_bank + wb))

        if fc_bed == 1:
            D_bank = 0
        else:
            D_bank = ((Qs_in) / reach_length) * (1 / (((k_dep) * (np.power(tau_bank, 3/2) / np.power(tau_bed, 3/2)) * ((1 - fc_bank) / (1 - fc_bed))) * wb + l_bank))
            #D_bank = ((Qs_in) / reach_length) * (1 / (((k_dep) * (np.power(tau_bank, 3/2) / np.power(tau_bed, 3/2))) * wb + l_bank))
        ...
        if time % save_interval == 0:
            ...
            which_regime_arr[iter] = which_regime
...
#        if np.isclose(np.average(prior_w), wb, rtol = rtol, atol = atol) and np.isclose(np.average(prior_S), S, rtol = rtol, atol = atol):#check to see if width and slope are at steady state
...
#        else:
#            prior_w = np.roll(prior_w, -1)
#            prior_w[-1] = wb
#            
#            prior_S = np.roll(prior_S, -1)
#            prior_S[-1] = S
    ...
    return (save_widths, save_slopes, save_depths, save_depths_r, save_qs_out, 
            ...
            save_chan_depths, teq, which_regime_arr)
